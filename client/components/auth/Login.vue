...
export default {
  ...
  data () {
    return {
      ...
      process: 'isSuccess',
      redirectTo: '',
      message: 'Thank you for successfully confirming your credentials! Please wait a little more, and pay less with Youkeep'
    }
  },
  computed: mapGetters({
    store: 'shop/store',
  ...
  methods: {
    ...
    async onLogin (params) {
      ...
      await this.$store.dispatch('auth/onPost', payload).catch((error) => {
        ...
        setTimeout(() => {
          if ($this.isValid) {
            $this.$message.success('Welcome back, ' + $this.user.name + '. Enjoy your business with Youkeep.')
          } else {
