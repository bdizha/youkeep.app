<template>
  <a-row class="r-text-left" :gutter="[48,48]" align="middle" justify="center" type="flex">
    <a-col :lg="{ span: 16 }" :md="{ span: 18 }"
           :sm="{ span: 24 }"
           :xs="{ span: 24 }"
    >
      <a-card class="r-bg-white r-pull-h-24">
        <a-row :gutter="[48,48]" align="top" justify="start" type="flex">
          <a-col :lg="{ span: 12 }" :md="{ span: 12 }"
                 :sm="{ span: 24 }"
                 :xs="{ span: 24 }"
          >
            <a-row :gutter="[24,24]" align="middle" justify="start" type="flex">
              <a-col :lg="{ span: 24 }" :md="{ span: 24 }"
                     :sm="{ span: 24 }"
                     :xs="{ span: 24 }"
              >
                <div class="r-shade-primary">
                  <h4 class="r-heading-light r-text-primary r-text-uppercase">
                    A smart way to invest
                  </h4>
                </div>
              </a-col>
              <a-col :lg="{ span: 24 }" :md="{ span: 24 }"
                     :sm="{ span: 24 }"
                     :xs="{ span: 24 }"
              >
                <p class="r-text-medium">
                  Our specialist property team will take care of all the hard work so you can sit back and watch your
                  portfolio grow.
                </p>
              </a-col>
              <a-col :lg="{ span: 24 }" :md="{ span: 24 }"
                     :sm="{ span: 24 }"
                     :xs="{ span: 24 }"
              >
                <p class="r-text-normal">
                  Simply start by choosing how much you’d like to invest.
                </p>
              </a-col>
              <a-col :lg="{ span: 24 }" :md="{ span: 24 }"
                     :sm="{ span: 24 }"
                     :xs="{ span: 24 }"
              >
                <a-collapse accordion
                            :defaultActiveKey="activeKey"
                            expandIconPosition="right"
                >
                  <a-collapse-panel v-for="(section, index) in sections"
                                    :key="(index + 1).toString()"
                                    :class="'r-collapse-panel-light'"
                                    class="r-collapse-panel"
                  >
                    <template slot="header">
                      <a-row :gutter="[12,12]" align="middle" justify="start" type="flex">
                        <a-col :lg="{ span: 24 }" :md="{ span: 24 }" :sm="{ span: 24 }" :xs="{ span: 24 }"
                        >
                          <p class="r-text-medium">
                            {{ section.title }}
                          </p>
                        </a-col>
                      </a-row>
                    </template>
                    <a-row :gutter="[24,24]" align="middle" justify="start" type="flex">
                      <a-col v-for="(item, index) in section.items"
                             :key="index"
                             :lg="{ span: 24 }" :md="{ span: 24 }" :sm="{ span: 24 }" :xs="{ span: 24 }"
                      >
                        <div class="r-checked-item">
                          <a-icon class="r-checked-item-icon" type="check"></a-icon>
                          {{ item.content }}
                        </div>
                      </a-col>
                    </a-row>
                  </a-collapse-panel>
                </a-collapse>
              </a-col>
            </a-row>
          </a-col>
          <a-col class="r-text-center" :lg="{ span: 8 }" :md="{ span: 8 }"
                 :sm="{ span: 24 }"
                 :xs="{ span: 24 }"
          >
            <r-avatar :data-src="'/images/content/illustration-16.svg'" :size="300"
                      class="r-avatar-block"
            ></r-avatar>
          </a-col>
        </a-row>
      </a-card>
    </a-col>
  </a-row>
</template>
<script>
export default {
  name: 'r-plan-auto',
  props: {
    size: { type: Number, required: false, default: 16 },
    hasMore: { type: Boolean, required: false, default: true }
  },
  data () {
    return {
      activeKey: 1,
      title: 'Spazastop connects shoppers to sellers.',
      content: 'Our shoppers handle all your order collection and delivery experience as well as returns for you at\n' +
        'a very low cost.',
      sections: [
        {
          title: 'Auto Invest features',
          items: [
            {
              content: 'Properties are chosen based on their growth potential'
            },
            {
              content: 'Our aim is to generate returns that outperform the South African property market. We measure this annually against the CoreLogic Home Value Index'
            },
            {
              content: 'Choose how much you’d like to invest monthly via direct debit'
            },
            {
              content: 'Ability to skip a month or cancel'
            },
            {
              content: 'Receive regular updates on how your portfolio is performing'
            }
          ]
        },
        {
          title: 'AI powered investment option',
          items: [
            {
              content: 'Our specialist property team carefully select a range of high growth potential properties'
            },
            {
              content: 'Set a monthly amount and Smart Invest buys and reinvests any distributions in Bricks that we expect to outperform the South African residential property market. We measure this annually against the CoreLogic Home Value Index'
            },
            {
              content: 'Choose how much you’d like to invest monthly via direct debit'
            },
            {
              content: 'We\'ll take care of everything else and you\'ll receive your cut of the rent, after expenses, automatically reinvested'
            },
            {
              content: 'As property prices change so does the value of your Bricks'
            },
            {
              content: 'When the time comes, you can put your Bricks up for sale'
            }
          ]
        }
      ]
    }
  },
  methods: {
    hasTitle () {
      return this.title.length > 0
    }
  }
}
</script>
